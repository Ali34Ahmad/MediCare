...
class MedicareAppViewModel @Inject constructor(
    ...
    fun updateCurrentDestination(destination: Destination){
        ...
        when(destination){
            ...
            is Destination.VaccinationAppointments->_uiState.value=_uiState.value.copy(selectedIndex = 1)
            is Destination.ClinicAppointments->_uiState.value=_uiState.value.copy(selectedIndex = 2)
